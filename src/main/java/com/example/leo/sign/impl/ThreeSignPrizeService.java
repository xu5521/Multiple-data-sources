 ...
 */
@Service("3SignPrizeService")
@Slf4j
public class ThreeSignPrizeService implements SignPrizeService {
    @Autowired
    private CheckConfigService checkConfigService;

    @Override
    public Long amount() {
        CheckConfig checkConfig = checkConfigService.getOne(Wrappers.<CheckConfig>lambdaQuery().eq(CheckConfig::getNum, 3));
