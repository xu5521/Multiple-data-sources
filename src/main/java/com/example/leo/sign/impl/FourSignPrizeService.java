 ...
 */
@Service("4SignPrizeService")
@Slf4j
public class FourSignPrizeService implements SignPrizeService {
    @Autowired
    private CheckConfigService checkConfigService;

    @Override
    public Long amount() {
        CheckConfig checkConfig = checkConfigService.getOne(Wrappers.<CheckConfig>lambdaQuery().eq(CheckConfig::getNum, 4));
